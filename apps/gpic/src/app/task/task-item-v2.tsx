...
export function TaskItemV2 ({task, children, className, ...rest}:BatchTaskItemProps) {
  // const
  const inputImages = task.input.files
  const f = task.executions?.[0].output?.url ?? task.input.files[0]
  return <>
    <div
      className={cn(
    ...
    >
      <div className={'max-h-32 flex'}>
        ...
        <div className={'relative inset-0 max-w-32'}>
          ...
          <div
            className={'absolute w-full flex flex-col items-start p-0.5 bottom-0 bg-gradient-to-b from-transparent to-black/60'}>
        ...
        <div
          className={'relative justify-center items-center  w-full flex overflow-hidden bg-gray-400/50 rounded-r-lg'}>
          <img src={f} className={
            cn(
          ...
          <div
            className={'absolute top-0 rounded-tr-lg flex flex-col items-end w-full text-xs p-1 bg-gradient-to-t from-transparent to-black/30'}>
            <StatusBadge status={task.status}/>
          </div>
          <div className={'absolute bottom-0 rounded-tr-lg justify-end flex gap-1 items-end w-full text-xs p-1 bg-gradient-to-b from-transparent to-black/30'}>
            <Button
              onClick={() => {

              }}
              size={'sm'} variant={'link'} className={'text-xs bg-transparent hover:bg-transparent text-white'}>
              重试
            </Button>
